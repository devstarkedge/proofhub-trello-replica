...
const SalesFilters = () => {
  ...
  const toggleFilters = useCallback(() => {
    setIsExpanded(prev => {
      const next = !prev;
      // If closing, remove focus from the button to avoid focus outlines/white flash
      if (prev) {
        setTimeout(() => filterBtnRef.current?.blur(), 80);
      }
      return next;
    });
  }, []);
  ...
  const labelClass = "block text-[11px] font-semibold text-gray-600 dark:text-gray-400 mb-1 uppercase tracking-wider";
    ...
    <div className="bg-gradient-to-r from-white via-blue-50/20 to-white dark:from-gray-800 dark:via-blue-900/10 dark:to-gray-800 border-b border-gray-200/80 dark:border-gray-700/80 shadow-sm backdrop-blur-sm">
      ...
      <div className="px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 max-w-2xl relative group">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm"
          />
          {searchInput && (
            <button
        ...
        <motion.button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-all shadow-sm hover:shadow-md focus:outline-none focus:ring-0 ${
        ...
        >
          ...
          <span>Filters</span>
          {activeFilterCount > 0 && (
            <span className="px-2 py-0.5 text-[11px] font-semibold bg-white/20 rounded-full min-w-[20px] text-center">
              {activeFilterCount}
            </span>
          )}
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            <div className="px-4 py-3 border-t border-gray-100/60 dark:border-gray-700/60 bg-gradient-to-b from-blue-50/20 to-transparent dark:from-blue-900/5 dark:to-transparent">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                
                {/* Date Presets Dropdown */}
                <div className={`col-span-1 sm:col-span-2 md:col-span-2 grid gap-2 ${datePreset === 'custom' ? 'grid-cols-3' : 'grid-cols-1'}`}>
                    <div>
