...
/** Shown only when there's at least one real difference — the caller (PoliciesTab) skips straight to saving nothing when the diff is empty. */
const PolicyDiffConfirmModal = ({ diff, onCancel, onConfirm, submitting }) => (
  <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4" onClick={onCancel}>
    ...
    >
      <div>
        <h3 className="text-base font-bold" style={{ color: 'var(--color-text-primary)' }}>Confirm Policy Changes</h3>
        <p className="mt-1 text-xs" style={{ color: 'var(--color-text-muted)' }}>
          Review what will change before saving. This creates a new policy version — nothing already accrued or approved is rewritten.
        </p>
      </div>

      <div className="space-y-2">
        {diff.map((row) => (
          <div key={row.label} className="rounded-lg border p-3 text-xs" style={{ borderColor: 'var(--color-border-default)' }}>
      ...
      <div className="flex justify-end gap-3 pt-2">
        <Button type="button" variant="outline" onClick={onCancel} disabled={submitting}>Back to editing</Button>
...
export default PolicyDiffConfirmModal;
